...
public class AdminServiceTest extends TestCase {
    ...
    public void testGetPartitionsAsStream() throws IOException {
        ...
        Store<byte[], byte[]> store = server.getStoreMap().get(storeName);
        ...
        AdminClient client = new AdminClient(server.getIdentityNode(),
                                             server.getMetaDataStore(),
                                             new SocketPool(100, 100, 2000));
        ArrayList<Entry<byte[], Versioned<byte[]>>> entryList = client.requestGetPartitionsAsStream(0,
                                                                                                    storeName,
                                                                                                    new int[] { 0 });
        ...
        for(Entry<byte[], Versioned<byte[]>> entry: entryList) {
            checkEntriesForPartitions(entry.getKey(), storeName, new int[] { 0 }, routingStrategy);
        }
        ...
        for(Entry<byte[], Versioned<byte[]>> entry: entryList) {
            checkEntriesForPartitions(entry.getKey(),
                                      storeName,
                                      new int[] { 0, 1 },
                                      routingStrategy);
            partitionSet2.add(routingStrategy.getPartitionList(entry.getKey()).get(0));
        }
    ...
    public void testPutEntriesAsStream() throws IOException {
        ...
        for(int i = 100; i <= 1000; i++) {
            ...
            entryList.add(new Entry<byte[], Versioned<byte[]>>(key, new Versioned<byte[]>(value)));
        }
        ...
        for(int i = 100; i <= 1000; i++) {
            assertNotSame("Store should return a valid value",
                          "value-" + i,
                          new String(store.get(ByteUtils.getBytes("" + i, "UTF-8"))
                                          .get(0)
                                          .getValue()));
        }
    }
    ...
    public void testPipeGetAndPutStreams() throws IOException {
        ...
        for(int i = 100; i <= 1000; i++) {
            ...
            if(routingStrategy.getPartitionList(key).get(0) == 0
               || routingStrategy.getPartitionList(key).get(0) == 1) {
                checked++;
