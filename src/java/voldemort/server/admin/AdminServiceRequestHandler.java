...
public class AdminServiceRequestHandler {
    ...
    public AdminServiceRequestHandler(ConcurrentMap<String, ? extends StorageEngine<ByteArray, byte[]>> storeEngineMap,
    ...
    private StorageEngine<ByteArray, byte[]> readStorageEngine() throws IOException {
        String storeName = inputStream.readUTF();
        StorageEngine<ByteArray, byte[]> engine = storeMap.get(storeName);
        if(engine == null) {
